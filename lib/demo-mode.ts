...
import episodesData from '@/data/mock/episodes.json'
import hostsData from '@/data/mock/hosts.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetEpisodes') || query.includes('nodeEpisodes')) {
      return loadMockData('episodes.json')
    }

    if (query.includes('GetHosts') || query.includes('nodeHosts')) {
      return loadMockData('hosts.json')
    }
